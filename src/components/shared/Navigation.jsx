...
import { Search, Calendar, User, PlusCircle, LayoutGrid } from 'lucide-react';
...
export default function Navigation({ role, activeTab, setActiveTab }) {
  const tabs = role === 'B2C' ? [
    ...
    { id: 'templates', label: 'Шаблони', icon: LayoutGrid },
    { id: 'profile', label: 'Профіль', icon: User }
  ];
    ...
    <div className={`fixed bottom-6 left-1/2 -translate-x-1/2 w-[calc(100%-32px)] max-w-[418px] bg-white dark:bg-zinc-800 shadow-xl dark:shadow-black/40 rounded-[32px] px-2 py-3 z-[100] flex justify-around items-center ${
      ...
      {tabs.map((tab) => {
        ...
        return (
          ...
          >
            <div className={`p-2 rounded-full transition-all duration-200 ${isActive ? 'bg-orange-50 dark:bg-orange-950/20 text-[#f97316]' : 'text-gray-400 dark:text-gray-500'}`}>
              <IconComponent size={18} />
            </div>
            <span className={`text-[9px] font-black mt-1 tracking-tight ${isActive ? 'text-[#f97316]' : 'text-gray-400 dark:text-gray-550'}`}>
              {tab.label}
            </span>
          </button>
        );
