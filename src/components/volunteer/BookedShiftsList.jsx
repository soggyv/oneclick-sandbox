...
export default function BookedShiftsList({
  ...
  return (
    <div className="animate-fadeIn text-left">
      <div className="mb-5">
        <h1 className="text-xl font-black tracking-tight text-gray-900 dark:text-zinc-100">Мої заходи</h1>
        <p className="text-[10px] text-gray-400 dark:text-zinc-500 font-bold uppercase tracking-wider">Список ваших запланованих робіт</p>
      </div>

      {/* Tabs Filter (АКТИВНІ / ЗАВЕРШЕНІ) */}
      <div className="flex border border-gray-100 dark:border-transparent mb-5 bg-white dark:bg-[#27272A] p-1 rounded-full shadow-sm">
      ...
      <div className="space-y-4">
        {filteredB2CBookedShifts.length > 0 ? (
          filteredB2CBookedShifts.map((app) => (
            ...
            >
              ...
              <h3
                ...
                className="font-black text-gray-900 dark:text-zinc-200 text-sm leading-snug mb-2 cursor-pointer hover:underline"
              >
                {app.shift.title}
              </h3>

              <div className="space-y-1 mb-4 text-[11px] text-gray-500 dark:text-zinc-400 font-semibold">
                <p className="flex items-center gap-1.5">
                  <Building2 size={12} className="text-gray-300 dark:text-zinc-600" />
              ...
              {app.status === 'approved' && (
                <div className="mt-3.5 space-y-2">
                  ...
                  {showQrCodes[app.id] && (
                    <div className="bg-orange-50/50 dark:bg-[#18181B] border border-orange-100 dark:border-transparent rounded-xl p-3.5 text-center animate-fadeIn">
                      <p className="text-[10px] text-gray-400 dark:text-zinc-500 font-bold uppercase tracking-wider mb-1">
                        Код для підтвердження присутності
                      </p>
                      <span className="text-base font-black text-gray-900 dark:text-[#F97316] tracking-widest select-all">
                        {app.check_in_code}
                      </span>
                      <p className="text-[9px] text-gray-400 dark:text-zinc-500 mt-1 font-semibold leading-relaxed">
              ...
              {(app.status === 'pending' || app.status === 'approved') && (
                <button
                  ...
                  onClick={(e) => {
                    e.stopPropagation();
                    if (window.confirm(`Ви дійсно бажаєте скасувати запис на зміну "${app.shift.title}"?`)) {
                      handleCancelShift(app.id);
                    }
                  }}
                  className="w-full mt-3 py-2 bg-red-50 hover:bg-red-100/60 dark:bg-red-950/30 dark:hover:bg-red-950/50 text-red-600 dark:text-red-400 font-bold text-[10px] rounded-full uppercase tracking-wider transition-all active:scale-95 flex items-center justify-center gap-1.5 cursor-pointer border border-red-100 dark:border-transparent"
                >
          ...
          <div className="bg-white dark:bg-[#27272A] rounded-2xl p-8 border border-gray-100 dark:border-transparent shadow-sm text-center flex flex-col items-center justify-center">
            ...
            <h4 className="font-bold text-gray-800 dark:text-zinc-200 text-xs mb-1">
              {activeB2CShiftsFilter === 'АКТИВНІ' ? 'Немає запланованих заходів' : 'Немає завершених заходів'}
            </h4>
            <p className="text-[10px] text-gray-400 dark:text-zinc-500 max-w-[180px] leading-relaxed">
