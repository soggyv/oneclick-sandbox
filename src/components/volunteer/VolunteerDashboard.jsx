...
import { Building2, Search, Clock, MapPin, RotateCw, Sun, Moon, LayoutGrid, List } from 'lucide-react';
            ...
            <div className="space-y-4">
              {shifts.map((shift) => (
                ...
                >
                  ...
                  <p className="text-[11px] text-gray-400 dark:text-zinc-550 font-bold mb-3 flex items-center gap-1.5">
            ...
            <div className="bg-white dark:bg-[#27272A] rounded-3xl border border-gray-200/50 dark:border-zinc-800/40 p-2.5 shadow-sm space-y-1">
              {shifts.map((shift) => (
                <div
                  key={shift.id}
                  onClick={() => setCurrentDetailsShift(shift)}
                  className="flex items-center justify-between py-3 px-4 hover:bg-gray-55 dark:hover:bg-zinc-900/40 rounded-2xl transition-all cursor-pointer border-b border-gray-100/50 dark:border-zinc-700/40 last:border-b-0 text-left"
                >
                  <div className="min-w-0 flex-1 pr-4">
                    <div className="flex items-center gap-2 mb-1">
                      <span className="px-2 py-0.5 bg-gray-50 dark:bg-zinc-800/80 text-[8px] font-extrabold text-gray-500 dark:text-zinc-400 rounded uppercase tracking-wider shrink-0 block">
                        {shift.category}
                      </span>
                      <h3 className="font-black text-gray-900 dark:text-zinc-200 text-xs sm:text-sm truncate leading-tight">
                        {shift.title}
                      </h3>
                    </div>
                    <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-[10px] font-bold text-gray-400 dark:text-zinc-550">
                      <span className="flex items-center gap-1">
                        <Building2 size={11} className="text-gray-300 dark:text-zinc-650" />
                        <span>{shift.organization_name}</span>
                      </span>
                      <span className="flex items-center gap-1">
                        <Clock size={11} className="text-[#FF5522] dark:text-[#F97316]" />
                        <span>{shift.time}</span>
                      </span>
                      <span className="flex items-center gap-1">
