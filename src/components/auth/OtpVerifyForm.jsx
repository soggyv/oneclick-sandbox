...
export default function OtpVerifyForm({
  ...
  return (
    <form onSubmit={handleVerifyOtp} className="space-y-4 w-full max-w-[320px] animate-fadeIn">
      <div>
        <label className="block text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest mb-1 px-1">
          Код підтвердження
        </label>
        <p className="text-[10px] text-gray-400 dark:text-gray-500 font-semibold mb-3 px-1 leading-relaxed">
          {regRole === 'B2B' ? (
            <>
              Ми надіслали 4-значний код на пошту <span className="text-gray-900 dark:text-white font-black">{regEmail}</span> (Код доступу: <span className="text-[#FF5522] font-black">{otpCode}</span>)
            ...
            <>
              Ми надіслали 4-значний код на номер <span className="text-gray-900 dark:text-white font-black">{regPhone}</span> (Код доступу: <span className="text-[#FF5522] font-black">{otpCode}</span>)
            </>
          )}
        </p>
        <input
      ...
      <button
        ...
        className="w-full py-3.5 bg-white dark:bg-zinc-800 hover:bg-gray-50 dark:hover:bg-zinc-700 dark:hover:text-white border border-gray-200 dark:border-zinc-700 text-gray-700 dark:text-gray-250 font-extrabold rounded-full shadow-sm text-xs tracking-wider uppercase transition-all active:scale-95 cursor-pointer flex items-center justify-center gap-2"
      >
        Назад
      </button>
    </form>
